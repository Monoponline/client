    ...
    <div className="Game">
      {props.started ? (
        ...
        <>
          ...
          <ul style={{ display: "flex", justifyContent: 'center', fontSize: '6rem', flexDirection: 'column', alignItems: 'center' }}>
            {props.players.map((player) => <li key={player.name}>{player.name}</li>)}
          </ul>
          {props.players.length >= 2 ? (
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <button style={{ fontSize: '6rem' }} onClick={() => socket.emit('player-start')}>Commencer la partie</button>
            </div>
          ) : <></>}
        </>
      )}
